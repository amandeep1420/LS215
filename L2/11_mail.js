...
> split the string into separate emails using ##||##; this is the count
> split each email into five parts using #/#
  - date is always at index 2
...
function mailCount(emaildata) {
  ...
  let dateStrings = emails.map(email => email.split('#/#')[2]);
  let dates       = dateStrings.map(string => string.replace(/[^\d\-]/g, ''));
  
  dates.sort(sortDates);
  
  let oldestDate = dates[0];
  let newestDate = dates[dates.length - 1];
...
// there is a toDateString method...lol...

// this is a good solution to study from the book; come back to it