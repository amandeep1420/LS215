...
function findNumericGrade(exams, exercises) {
  ...
  let examsAvg     = exams.reduce((sum, score) => sum + score) / exams.length;
  let exerciseAvg  = exercises.reduce((total, score) => total + score);

  let finalGrade = (examsAvg * EXAM_WEIGHT) + (exerciseAvg * EXERCISE_WEIGHT);
